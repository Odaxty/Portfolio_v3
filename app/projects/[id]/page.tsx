...
export default function ProjectDetail() {
  ...
  if (!project) {
    return (
      <div className="text-white text-center mt-20">Projet introuvable 😢</div>
    );
  }
        ...
        <div className="absolute inset-0 z-10 pointer-events-none">
          ...
          <div className="absolute bottom-0 left-0 w-full p-6 md:p-10">
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              <span className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 text-white shadow-lg">
                🗓️ {project.year}
              </span>
              {project.duration && (
                <span className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 text-white shadow-lg">
              ...
              <>
                ...
                <div className="space-y-3">
                  {project.features.map((feature, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-4 p-4 bg-neutral-900 rounded-xl border border-neutral-800 hover:border-green-500/50 transition-all group"
                    >
                      <span className="w-8 h-8 flex items-center justify-center rounded-lg bg-green-500/10 text-green-500 font-bold group-hover:bg-green-500 group-hover:text-black transition-colors">
                        {index + 1}
                      </span>
                      <div>
            ...
            <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800 shadow-lg">
              ...
              <div className="flex flex-wrap gap-2">
                {project.tech.map((tech) => (
                  <span
                    key={tech}
                    className="bg-neutral-800 px-3 py-1.5 rounded-lg text-sm text-gray-300 border border-neutral-700"
                  >
                    {tech}
                  </span>
                ))}
